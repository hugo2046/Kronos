...
**门禁口径注记（2026-08-10，双基准定稿）**：计划 §2.2.5 / §4 规则1 原文写的是
"占位 |AER|<3%"（对称阈值）。但同一计划 §0 锁定了单边 0.15% 成本 + 每日调仓 +
n=5/k=50，且实测发现：等权 top-50 组合天然带约 +4.4% 的等权-beta 溢价
（2024-07~2025-06 全池等权 +17.6% vs csi300 市值加权指数 +13.2%）。故用 csi300
指数基准判门禁时，随机信号也会"复现" +4.3% AER——这是基准结构差不是 alpha。

门禁改用**同池等权基准**：剥离掉等权-beta 溢价后，随机占位 AER≈0（实测 +0.02%），
才是"引擎不制造选股 alpha"的干净判据。本测试在合成数据上用等权基准（本就同分布），
直接验证门禁意图。负的成本拖（约 -6%）来自真实换手成本，是预期内的。

判读 K/M/R 组的锚点比较（规则 2）与定性判定（规则 3）仍用 csi300 指数基准（论文口径），
与论文 base 锚点（Table 10 CSI300：AER 0.1911 / IR 1.3782）同口径比较。
"""
