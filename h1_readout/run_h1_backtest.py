"""H1 引擎全表封存（计划 §3.3，**落盘不判读，一次开封**）。

canonical 引擎（csi300, k=50/n=5/min_hold=5/15bp，双基准）两窗 ×（四 H1 臂 +
R1 锚两行（R-lin_s42/R-kda_s42，r1_objective parquet 只读复跑）+ 在位者 G1_mean +
F0×4 / M 参照）。引擎宇宙与 g5/r1 一致（冻结口径轮次并集——在位者与 R1 锚
复跑一致性的门禁前提）。

**纪律 §4**：绩效数字只写封存日志 ``h1_readout/data/h1_backtest_sealed.log``
与结果 JSON（judge 之前不读不贴）；终端只打印结构信息。

用法：``/home/user/miniconda3/envs/quant/bin/python -m h1_readout.run_h1_backtest --seed 42``
"""
...
# —— 纪律 §4：移除 console sink，数字只落封存日志 ——
logger.remove()

PKG_DIR = Path(__file__).resolve().parent
DATA_DIR = PKG_DIR / "data"
R4 = PKG_DIR.parent / "finetune_suite" / "data"
R1_DATA = PKG_DIR.parent / "r1_objective" / "data"
...
# R1 锚（s42 两行；judge 时与 r1_backtest_results.json 冻结值对拍）
R1_ANCHORS = ("R-lin_s42", "R-kda_s42")
...
def _load_window_signals(window: str, seed: int) -> dict[str, pd.DataFrame]:
    ...
    for n in R1_ANCHORS:
        signals[n] = pd.read_parquet(R1_DATA / f"daily_signals_{window}_{n}.parquet")
    for arm in H1_ARMS:
        p = DATA_DIR / arm / f"daily_signals_{window}_{arm}_s{seed}.parquet"
        assert p.exists(), f"H1 信号缺失：{p}（先跑 run_h1_signals）"
        signals[f"{arm}_s{seed}"] = pd.read_parquet(p)
    return signals
...
def main() -> None:
    ...
    sealed_log = DATA_DIR / "h1_backtest_sealed.log"
    logger.add(str(sealed_log), enqueue=False)

    h1_names = [f"{a}_s{seed}" for a in H1_ARMS]
    results: dict[str, dict] = {n: {} for n in h1_names + list(R1_ANCHORS)}
    ...
    for window, (start, end) in WINDOW_BOUNDS.items():
        cfg = replace(BaselineConfig.load(window="oos"),
                      window=f"h1_{window}", backtest_start=start, backtest_end=end)
        signals = _load_window_signals(window, seed)

        first = signals[h1_names[0]]
        assert signals["G1_mean"].index.equals(first.index), (
            f"[{window}] 在位者索引与 H1 打分网格不一致")
        ...
        if window == "backtest":
            ...
            logger.info(f"对拍在位者 G1_mean：复跑 {rerun:+.4%} vs 冻结 "
                        f"{INCUMBENT_AER_EW:+.4%} 一致")
        n_days_total += len(rebalances)

    out = {
        "experiment": "h1_backtest_sealed", "seed": seed, "date": "2026-09-05",
        ...
        "results": results,
        "incumbent": incumbent,
        "note": "纪律 §4：数字落盘后不判读不外泄，统一 run_h1_judge 一次开封（HC1~HC4）",
    }
    out_path = DATA_DIR / "h1_backtest_results.json"
    out_path.write_text(json.dumps(out, indent=2, ensure_ascii=False, default=str))

    print(f"H1 引擎出表完成：{len(h1_names)} H1 臂 + {len(R1_ANCHORS)} R1 锚 + 在位者，"
          f"{len(WINDOW_BOUNDS)} 窗 × 双基准，{n_days_total} 日")
