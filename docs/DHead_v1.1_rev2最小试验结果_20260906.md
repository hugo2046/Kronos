...
- 诊断性观察（仅本批 8 日样本含义，无统计检验）：B（R2 仿射还原接口）
  **本批样本数值优于** A（旧 raw-return 接口）——初始 D 5.60 vs 158.93
  （约 28×），末点 D 0.031 vs 0.053，逐日 Spearman(vs 教师 r0) 0.827 vs
  0.636；两臂均数值优于可部署的全局逐期限均值基线（E=0.0884）。
  **观察到 B 在训练样本上的排序相关与截面方差（2.5e-4，非恒定），但尚不
  能确认该排序相对接口基线（z=0 / 未训练头）的训练增量与泛化**——归因
  分离见零训练诊断文档。A/B 差异无泛化/收益/main 解锁含义。
...
| 末 epoch 梯度范数（**裁剪前**，clip=1 已执行） | 54.4 | 4.19 |
| 非有限值累计 | 0 | 0 |
...
A 臂梯度范数（54，clip 前值）大于 B（4.2）且曲线在 ep199 仍未见平台——与
"接口尺度病态使 A 优化困难"的解读一致，但属诊断观察，非结论。

## 4. 允许结论边界（rev2 §3.3 冻结代入）

1. 两臂均未过工程检查（D≤R_train）→ **"本配置在本预算下未通过最小拟合
   检查，原因待诊断"**。B 臂距阈值 10%；观察到训练样本排序相关，但
   尚不能确认相对接口基线的训练增量与泛化，不得声称逐股信息已学会；
   也不判容量不足/架构无效/扩大搜索。
...
- 真实资产预检（试验前）：G1 eval 断言 OK；R2 真实数据对拍（64 样本×
  z=0.7）最大偏差 1.7e-16。**精度口径（复核 20260906 §4.3）**：该值
  为 float64 公式自洽精度；KronosPredictor 内部张量为 float32、教师
  分片 float32 落盘，学生还原链同为 float32——FP32 真实链路的舍入差
  由零训练诊断的容差检查单独量化，1.7e-16 不等于 FP32 预测链逐位等价。
...
# 本轮实跑 exit 0。**耗时口径（复核 20260906 §4.3，分项列示）**：
#   教师首生成 171.7s + A 臂训练 94.8s + B 臂训练 96.1s = 362.6s（全轮累计）；
#   195s 为命中教师缓存后的最终成功重跑 wall time（不含首生成与两次失败尝试，
#   失败尝试耗时日志未单独计时，如实标注未知；不补账重跑）
```
